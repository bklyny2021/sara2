...
Fix missing imports and optimize advanced system data collection
"""
...
class SaraSeniorTechnician:
    def __init__(self):
        ...
        self.senior_questions = {
            ...
            "system_internals": [
                ...
                "What are the current interrupt affinity distribution across CPU cores?",
    ...
    def get_advanced_system_data(self, request_type):
        ...
        try:
            if any(keyword in request_type.lower() for keyword in ['iptables', 'firewall', 'netfilter']):
                try:
                    result = subprocess.run(['iptables', '-L', 'INPUT', '-v'], capture_output=True, text=True, timeout=5)
    ...
    def respond_senior_technician(self, message):
        ...
        elif any(keyword in msg_lower for keyword in ['what are you', 'your name', 'models']):
